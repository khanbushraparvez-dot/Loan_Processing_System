...
function getVisibleCases(session) {
  ...
  if (session.role === "Vendor Employee") return all.filter(c => c.employeeId === session.id || c.employeeEmail === session.email || c.createdBy === session.id);
...
function LoginPage({ onLogin }) {
  ...
  const [vendorStep, setVendorStep] = useState("credentials");
  const [otp, setOtp] = useState("");
  const [otpToken, setOtpToken] = useState("");
  const [pendingProfile, setPendingProfile] = useState(null);

  const resetLogin = () => {
    setError(""); setEmail(""); setPassword(""); setVendorStep("credentials");
    setOtp(""); setOtpToken(""); setPendingProfile(null); setLoading(false);
  };
  ...
  const finishLogin = async (authUser, profile) => {
    ...
    const user = {
      ...
      username: (authUser.email || profile.email || "").split("@")[0],
      role: profile.role,
      bankCode: code || "",
      bankName: bank?.name || profile.bank_name || "",
    ...
    DB.session = user;
    DB.audit("LOGIN", user.id, { email: user.email, role: user.role, bankCode: user.bankCode, branch: user.branch });
    try { await supabase.from("audit_logs").insert({ user_id: user.id, action: "LOGIN", details: { email: user.email, role: user.role, bankCode: user.bankCode, branch: user.branch } }); } catch {}
    onLogin(user);
  };
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      if (profile.role === "Vendor Admin") {
        setPendingProfile(profile);
        const token = await requestEmailOtp(e, "vendor-admin-login");
        setOtpToken(token); setOtp(""); setVendorStep("otp");
        return;
      }
      await finishLogin(data.user, profile);
    } catch (err) {
  ...
  const verifyVendorAdminOtp = async () => {
    ...
    setLoading(true);
    try {
      await verifyEmailOtp(email, otp, otpToken);
      const { data: { user: authUser } } = await supabase.auth.getUser();
      if (!authUser || !pendingProfile) throw new Error("Login session expired. Please login again.");
      await finishLogin(authUser, pendingProfile);
  ...
  return <div style={{minHeight:"100vh",fontFamily:"'Inter','Segoe UI',sans-serif",background:C.pageBg,display:"flex",flexDirection:"column"}}>
    <header style={{height:78,background:C.white,borderBottom:`1px solid ${C.gray200}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 34px",boxSizing:"border-box",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}><img src="/assets/ar-skeil.jpg" alt="AR SKEIL" style={{height:48,width:78,objectFit:"cover",objectPosition:"center",borderRadius:8}} /><div><div style={{fontWeight:900,color:C.dark,fontSize:16}}>ARSKEIL SERVICES LLP</div><div style={{fontSize:13,color:C.gold,fontWeight:800,letterSpacing:.3}}>Powered by श्री दिनेश एंटरप्राइजेज</div></div></div>
      <div style={{display:"flex",gap:8,alignItems:"center",background:C.gray100,padding:5,borderRadius:10}}><button type="button" onClick={()=>{resetLogin();setAuthView("login")}} style={{padding:"9px 18px",borderRadius:7,border:"none",background:C.dark,color:C.white,fontWeight:800,cursor:"pointer"}}>Login</button><button type="button" onClick={()=>{setError("");setAuthView("signup")}} style={{padding:"9px 18px",borderRadius:7,border:"none",background:"transparent",color:C.dark,fontWeight:800,cursor:"pointer"}}>Create New User</button></div>
    </header>
    <main style={{flex:1,display:"flex",minHeight:0}}><BrandPanel /><div style={{width:500,maxWidth:"100%",background:C.white,display:"flex",flexDirection:"column",justifyContent:"center",padding:"42px 46px",boxSizing:"border-box"}}>
      ...
      {vendorStep === "credentials" && <div style={{marginBottom:18}}><label style={labelStyle}>Official Email</label><input type="email" value={email} onChange={e=>{setEmail(e.target.value);setError("")}} style={inputStyle} placeholder="your.official@email.com" autoComplete="email" /><label style={{...labelStyle,marginTop:15}}>Password</label><div style={{position:"relative"}}><input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...inputStyle,paddingRight:44}} placeholder="Enter password" autoComplete="current-password"/><span onClick={()=>setShowPwd(!showPwd)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",cursor:"pointer"}}>{showPwd?"🙈":"👁️"}</span></div><button onClick={doLogin} disabled={loading} style={{width:"100%",padding:13,marginTop:18,background:loading?C.gray300:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:900,fontSize:15,cursor:loading?"not-allowed":"pointer"}}>{loading?"⏳ Signing in…":"Login →"}</button></div>}
      {vendorStep === "otp" && <div style={{marginBottom:18}}><div style={{background:C.greenBg,color:C.green,padding:"9px 12px",borderRadius:8,fontSize:12,fontWeight:800,marginBottom:14}}>✓ Vendor Admin password accepted. OTP sent to your official email.</div><label style={labelStyle}>Email OTP</label><input style={{...inputStyle,letterSpacing:4,textAlign:"center",fontSize:18}} inputMode="numeric" maxLength={6} value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,""))} onKeyDown={e=>e.key==="Enter"&&verifyVendorAdminOtp()} placeholder="6-digit OTP" autoComplete="one-time-code"/><button onClick={verifyVendorAdminOtp} disabled={loading} style={{width:"100%",padding:13,marginTop:10,background:C.dark,color:C.white,border:"none",borderRadius:8,fontWeight:900,cursor:loading?"not-allowed":"pointer"}}>{loading?"⏳ Verifying…":"Verify OTP & Login"}</button><button onClick={async()=>{await supabase.auth.signOut();resetLogin()}} style={{width:"100%",padding:9,marginTop:8,background:"transparent",color:C.gold,border:"none",fontWeight:800,cursor:"pointer"}}>← Start again</button></div>}
  ...
  if(success) return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:24}}>
    <div style={{background:C.white,borderRadius:16,padding:40,width:540,maxWidth:"100%",boxShadow:"0 8px 32px #0002",textAlign:"center"}}>
      ...
      <p style={{color:C.gray500,fontSize:14,lineHeight:1.6}}>
        Your official email has been verified. {isVendorAdmin?"Your Vendor Admin account is ready. Sign in using your password and the email OTP.":isVendorEmployee?"Your Vendor Employee request has been sent to Admin for approval. You will be able to log in after approval.":"Your Banker account is ready for login."}
      </p>
...
function ForgotPage({ onBack }) {
  const [email,setEmail]=useState(""); const [error,setError]=useState(""); const [loading,setLoading]=useState(false); const [done,setDone]=useState(false);
  const send=async()=>{setError("");const e=email.trim().toLowerCase();if(!/^\S+@\S+\.\S+$/.test(e)){setError("Please enter a valid official email address.");return;}setLoading(true);try{const {error}=await supabase.auth.resetPasswordForEmail(e,{redirectTo:window.location.origin});if(error)throw error;setDone(true);}catch(err){setError(err.message||"Unable to send password reset email.");}finally{setLoading(false);}};
  return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20}}><div style={{background:C.white,borderRadius:16,padding:38,width:430,maxWidth:"100%",boxShadow:"0 8px 32px #0002"}}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:22}}><span onClick={onBack} style={{cursor:"pointer",color:C.gold,fontSize:22,fontWeight:700}}>←</span><h2 style={{margin:0,fontWeight:900,fontSize:22,color:C.dark}}>Forgot Password</h2></div>{error&&<div style={{background:"#fee2e2",color:C.red,padding:"10px 14px",borderRadius:8,marginBottom:14,fontSize:13}}>⚠️ {error}</div>}{done?<><div style={{background:C.greenBg,color:C.green,padding:16,borderRadius:10,marginBottom:18,fontWeight:700}}>✓ Password reset email sent. Open the email and follow the link to create a new password.</div><button onClick={onBack} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer"}}>Back to Login</button></>:<><label style={labelStyle}>Official Email</label><input style={inputStyle} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your.official@email.com"/><button onClick={send} disabled={loading} style={{width:"100%",padding:13,background:loading?C.gray300:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:loading?"not-allowed":"pointer",marginTop:14}}>{loading?"Sending…":"Send Reset Email"}</button></>}</div></div>;
}
function Sidebar({ active, setPage, session, onLogout }) {
  const isBanker = session?.role === "Banker";
  const canBankerSee = key => !isBanker || session?.permissions?.[key] === true;
  const NAV = [
  ...
  const TOOLS = [
    ...
    ...(canBankerSee("receivedDocuments") ? [{ id: "receivedDocuments", icon: "📥", label: "Received Documents" }] : []),
    ...(canBankerSee("aiDocuments") ? [{ id: "aiDocuments", icon: "🤖", label: "AI Documents" }] : []),
    ...(canBankerSee("challan") ? [{ id: "challan", icon: "📄", label: "Challan" }] : []),
    ...(canBankerSee("noi") ? [{ id: "noi", icon: "ℹ️", label: "Notice of Intimation" }] : []),
  ];
  const REPORTS = [
    ...(canBankerSee("mis") ? [{ id: "mis", icon: "📋", label: "MIS Report" }] : []),
    { id: "payment", icon: "💳", label: "Payment Tracking" },
    ...(session?.role === "Vendor Admin" ? [{ id: "admin", icon: "⚙️", label: "Admin Panel" }] : [])
  ];
...
function AdminPanel({ session }) {
  const [users,setUsers]=useState([]); const [requests,setRequests]=useState([]); const [logs,setLogs]=useState([]); const [tab,setTab]=useState("requests"); const [loading,setLoading]=useState(false);
  const roleColor={"Vendor Admin":C.indigo,"Vendor Employee":C.green,Banker:C.sky};
  const permissionKeys=["receivedDocuments","aiDocuments","challan","noi","mis"];
  const permissionLabels={receivedDocuments:"Received Documents",aiDocuments:"AI Documents",challan:"Challan",noi:"NOI",mis:"MIS Report"};
  const refresh=async()=>{ const [{data:u},{data:r},{data:l}]=await Promise.all([supabase.from("profiles").select("*"),supabase.from("access_requests").select("*").eq("status","Pending").order("requested_at",{ascending:false}),supabase.from("audit_logs").select("*").order("created_at",{ascending:false}).limit(50)]); setUsers(u||[]);setRequests(r||[]);setLogs(l||[]); };
  useEffect(()=>{refresh();},[]);
  ...
  return <div><h2 style={{fontWeight:800,fontSize:22,color:C.dark,marginBottom:6}}>⚙️ Vendor Admin Panel</h2><p style={{color:C.gray500,fontSize:13,marginTop:0,marginBottom:20}}>Manage approvals, banker access, page permissions and account status.</p>
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>{[["requests",`🔔 Access Requests${requests.length?` (${requests.length})`:""}`],["users","👥 Users"],["audit","📋 Audit Logs"]].map(([t,l])=><button key={t} onClick={()=>setTab(t)} style={{padding:"8px 18px",background:tab===t?C.gold:C.white,color:tab===t?C.dark:C.gray600,border:`1px solid ${tab===t?C.gold:C.gray300}`,borderRadius:8,fontWeight:700,cursor:"pointer",fontSize:14}}>{l}</button>)}</div>
    {tab==="requests"&&<div>{requests.length===0?<div style={{background:C.white,borderRadius:12,border:`1px solid ${C.gray200}`,padding:"55px 30px",textAlign:"center"}}><div style={{fontSize:42}}>✅</div><div style={{fontWeight:800,color:C.dark,fontSize:17}}>No pending access requests</div></div>:<div style={{display:"grid",gap:14}}>{requests.map(r=><div key={r.id} style={{background:C.white,borderRadius:12,border:`1px solid ${C.gray200}`,padding:20}}><div style={{fontWeight:800,fontSize:16,color:C.dark}}>{r.name||r.email} <span style={{color:C.gray500,fontSize:12}}>({r.role})</span></div><div style={{color:C.gray600,fontSize:13,marginTop:4}}>{r.email}</div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginTop:16,background:C.gray100,borderRadius:8,padding:12}}><div><div style={{color:C.gray500,fontSize:10,fontWeight:700}}>BRANCH</div><div style={{fontSize:13,fontWeight:700}}>{r.branch||"—"}</div></div><div><div style={{color:C.gray500,fontSize:10,fontWeight:700}}>BANK</div><div style={{fontSize:13,fontWeight:700}}>{r.bank_name||"—"}</div></div><div><div style={{color:C.gray500,fontSize:10,fontWeight:700}}>REQUESTED</div><div style={{fontSize:13}}>{r.requested_at?new Date(r.requested_at).toLocaleString("en-IN"):"—"}</div></div></div><div style={{display:"flex",gap:10,marginTop:14}}><button onClick={()=>approveRequest(r)} style={{flex:1,padding:10,background:C.green,color:C.white,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer"}}>✓ Approve</button><button onClick={()=>rejectRequest(r)} style={{flex:1,padding:10,background:"#fee2e2",color:C.red,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer"}}>✕ Reject</button></div></div>)}</div>}</div>}
    ...
    {tab==="audit"&&<div style={{background:C.white,borderRadius:12,border:`1px solid ${C.gray200}`,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.gray100}}>{["Action","User","Details","Time"].map(h=><th key={h} style={{padding:"10px 16px",textAlign:"left",fontSize:11,color:C.gray500}}>{h}</th>)}</tr></thead><tbody>{logs.length===0?<tr><td colSpan={4} style={{padding:40,textAlign:"center",color:C.gray400}}>No audit logs yet.</td></tr>:logs.map(l=><tr key={l.id} style={{borderBottom:`1px solid ${C.gray100}`}}><td style={{padding:"10px 16px"}}>{l.action}</td><td style={{padding:"10px 16px",fontSize:13}}>{l.user_id||"—"}</td><td style={{padding:"10px 16px",fontSize:12,color:C.gray600}}>{JSON.stringify(l.details||{})}</td><td style={{padding:"10px 16px",fontSize:12,color:C.gray500}}>{l.created_at?new Date(l.created_at).toLocaleString("en-IN"):"—"}</td></tr>)}</tbody></table></div>}
  </div>;
}
