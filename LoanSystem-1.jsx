...
function BrandPanel() {
  return <div style={{flex:1,background:`linear-gradient(135deg, ${C.dark} 0%, #0b1020 100%)`,display:"flex",flexDirection:"column",justifyContent:"center",padding:"48px 60px",position:"relative",overflow:"hidden",minWidth:0}}>
    <div style={{position:"absolute",top:-120,right:-100,width:360,height:360,borderRadius:"50%",background:`${C.gold}10`,border:`1px solid ${C.gold}20`}} />
    <div style={{position:"relative",zIndex:1}}>
      <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:30}}><div style={{width:52,height:52,borderRadius:14,background:C.gold,display:"flex",alignItems:"center",justifyContent:"center",fontSize:25}}>🏦</div><div><div style={{color:C.gold,fontWeight:900,fontSize:22}}>ARSKEIL SERVICES LLP</div><div style={{color:C.gray400,fontSize:12,letterSpacing:1}}>Powered by श्री दिनेश एंटरप्राइजेज</div></div></div>
      ...
      <p style={{color:C.gray400,fontSize:15,lineHeight:1.7,maxWidth:470,margin:"0 0 30px"}}>A unified platform for bank-wise document processing, NOI lifecycle management, challans and case tracking.</p>
      <div style={{display:"inline-flex",alignItems:"center",gap:12,padding:"12px 16px",borderRadius:12,background:"#ffffff0d",border:"1px solid #ffffff14",color:C.gray200}}><div style={{fontSize:30}}>🔄</div><div><div style={{fontWeight:900,fontSize:13}}>NOI LIFECYCLE MANAGEMENT</div><div style={{fontSize:11,color:C.gray400,marginTop:3}}>Document → Processing → NOI → Registration → Completion</div></div></div>
    </div>
  </div>;
}
...
function LoginPage({ onLogin }) {
  ...
  const doLogin = async () => {
    ...
    if(!user){ const pending=DB.get("access_requests").find(r=>r.email?.toLowerCase()===e&&r.status==="Pending"); setError(pending?"Your account is waiting for Admin approval.":"No verified approved account found for this email."); return; }
    setLoading(true); await new Promise(r=>setTimeout(r,250));
    ...
    DB.session={...user,bankCode:code,bankName:bank.name,loginAt:new Date().toISOString()}; onLogin(DB.session); setLoading(false);
  };
  ...
  return <div style={{minHeight:"100vh",fontFamily:"'Inter','Segoe UI',sans-serif",background:C.pageBg,display:"flex",flexDirection:"column"}}>
    <header style={{height:72,background:C.white,borderBottom:`1px solid ${C.gray200}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 34px",boxSizing:"border-box",flexShrink:0}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{width:42,height:42,borderRadius:11,background:C.gold,display:"flex",alignItems:"center",justifyContent:"center",fontSize:21}}>🏦</div>
        <div><div style={{fontWeight:900,color:C.dark,fontSize:16}}>ARSKEIL SERVICES LLP</div><div style={{fontSize:10,color:C.gray500,letterSpacing:.7}}>Powered by श्री दिनेश एंटरप्राइजेज</div></div>
      </div>
      <div style={{display:"flex",gap:8,alignItems:"center",background:C.gray100,padding:5,borderRadius:10}}>
        <button type="button" onClick={()=>{setError("");setAuthView("login")}} style={{padding:"9px 18px",borderRadius:7,border:"none",background:authView==="login"?C.dark:"transparent",color:authView==="login"?C.white:C.gray600,fontWeight:800,cursor:"pointer"}}>Login</button>
    ...
    <main style={{flex:1,display:"flex",minHeight:0}}>
      ...
      <div style={{width:500,maxWidth:"100%",background:C.white,display:"flex",flexDirection:"column",justifyContent:"center",padding:"42px 46px",boxSizing:"border-box"}}>
        ...
        <div style={{marginBottom:16}}><label style={labelStyle}>Official Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={inputStyle} placeholder="your.official@email.com" autoComplete="email" /></div>
        <div style={{marginBottom:22}}><label style={labelStyle}>Password</label><div style={{position:"relative"}}><input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...inputStyle,paddingRight:44}} placeholder="Enter password" autoComplete="current-password"/><span onClick={()=>setShowPwd(!showPwd)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",cursor:"pointer"}}>{showPwd?"🙈":"👁️"}</span></div></div>
        <button onClick={doLogin} disabled={loading} style={{width:"100%",padding:13,background:loading?C.gray300:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:900,fontSize:15,cursor:loading?"not-allowed":"pointer"}}>{loading?"⏳ Signing in…":"Login →"}</button>
        <div style={{display:"flex",justifyContent:"space-between",marginTop:16}}><span onClick={()=>{setError("");setAuthView("forgot")}} style={{color:C.gold,fontSize:13,cursor:"pointer",fontWeight:700}}>Forgot password?</span><span onClick={()=>{setError("");setAuthView("signup")}} style={{color:C.gold,fontSize:13,cursor:"pointer",fontWeight:800}}>Don't have an account? Sign up</span></div>
      </div>
    </main>
  </div>;
}
...
function SignupPage({ onBack }) {
  ...
  const [form,setForm]=useState({name:"",bankCode:"",vertical:"",branch:"",email:"",otp:"",password:"",confirm:""});
  const [otp,setOtp]=useState(""); const [otpSent,setOtpSent]=useState(false); const [verified,setVerified]=useState(false); const [error,setError]=useState(""); const [success,setSuccess]=useState(false); const [loading,setLoading]=useState(false);
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  const selectedBank=getBank(form.bankCode);
  const isBanker=userType==="Banker";
  const isVendor=userType.startsWith("Vendor");
  const domain=isBanker?selectedBank?.emailDomain:(isVendor?EMAIL_DOMAINS.VENDOR:"");
  const domainValid=!!form.email.trim() && !!domain && emailMatchesDomain(form.email,domain);

  const resetVerification=()=>{setOtpSent(false);setVerified(false);setOtp("");setForm(f=>({...f,otp:""}));};
  const sendOtp=()=>{
    setError("");
    ...
    if(!domain){setError("Please select your bank so the official email domain can be verified.");return;}
    if(!domainValid){setError(`Please use an official email ending with ${domain}.`);return;}
    ...
    const code=makeOtp(); setOtp(code); setOtpSent(true); setVerified(false); setForm(f=>({...f,otp:""}));
    console.log("Prototype email OTP:",code);
  };
  const verifyOtp=()=>{setError("");if(form.otp.trim()===otp&&otpSent){setVerified(true);}else setError("Invalid OTP. Please enter the latest 6-digit code.")};
  const doSignup=async()=>{
    ...
    if(!form.email.trim()||!domainValid){setError(`Official email must end with ${domain||"the selected bank domain"}.`);return;}
    ...
    const role=userType;
    const base={name:form.name.trim(),username:form.email.trim().split("@")[0],email:form.email.trim(),passwordHash:hash,role,bankCode:form.bankCode,bankName:selectedBank.name,vertical:form.vertical,branch:form.branch.trim(),emailVerified:true};
    if(role==="Vendor Admin") {
      ...
      DB.audit("VENDOR_ADMIN_CREATED",user.id,{email:user.email,bankCode:user.bankCode});
    } else {
      const request={id:`req_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,...base,status:"Pending",requestedAt:new Date().toISOString()};
      DB.insert("access_requests",request); DB.audit("ACCESS_REQUEST",request.id,{email:request.email,role,bankCode:request.bankCode});
    }
    setLoading(false); setSuccess(true);
  };
  if(success) return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:24}}><div style={{background:C.white,borderRadius:16,padding:40,width:540,maxWidth:"100%",boxShadow:"0 8px 32px #0002",textAlign:"center"}}><div style={{fontSize:45}}>✅</div><h2 style={{margin:"10px 0 8px",color:C.dark}}>Registration successful</h2><p style={{color:C.gray500,fontSize:14,lineHeight:1.6}}>Your official email has been verified. {userType==="Vendor Admin"?"Your Vendor Admin account is ready for login.":"Your access request has been sent to Admin for approval."}</p><button onClick={onBack} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer",marginTop:12}}>← Back to Login</button></div></div>;
  ...
  return <div style={{minHeight:"100vh",background:C.pageBg,fontFamily:"'Inter','Segoe UI',sans-serif"}}>
    <header style={{height:72,background:C.white,borderBottom:`1px solid ${C.gray200}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 34px",boxSizing:"border-box"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:42,height:42,borderRadius:11,background:C.gold,display:"flex",alignItems:"center",justifyContent:"center",fontSize:21}}>🏦</div><div><div style={{fontWeight:900,color:C.dark,fontSize:16}}>ARSKEIL SERVICES LLP</div><div style={{fontSize:10,color:C.gray500,letterSpacing:.7}}>Powered by श्री दिनेश एंटरप्राइजेज</div></div></div>
      <div style={{display:"flex",gap:8,alignItems:"center",background:C.gray100,padding:5,borderRadius:10}}><button type="button" onClick={onBack} style={{padding:"9px 18px",borderRadius:7,border:"none",background:"transparent",color:C.dark,fontWeight:800,cursor:"pointer"}}>Login</button><button type="button" style={{padding:"9px 18px",borderRadius:7,border:"none",background:C.dark,color:C.white,fontWeight:800,cursor:"default"}}>Create New User</button></div>
    </header>
    <div style={{maxWidth:920,margin:"28px auto",padding:"0 18px",boxSizing:"border-box"}}>
      <div style={{background:C.white,borderRadius:16,padding:"30px 34px",boxShadow:"0 8px 32px #0001",border:`1px solid ${C.gray200}`}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:22}}><span onClick={onBack} style={{cursor:"pointer",color:C.gold,fontSize:22,fontWeight:700}}>←</span><div><h2 style={{margin:0,fontWeight:900,fontSize:25,color:C.dark}}>Create New User</h2><p style={{margin:"4px 0 0",color:C.gray500,fontSize:12}}>Choose your user type and verify your official email.</p></div></div>
        ...
        <div style={{marginBottom:22}}><label style={labelStyle}>Sign in as *</label><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>{["Banker","Vendor Admin","Vendor Employee"].map(t=><button type="button" key={t} onClick={()=>{setUserType(t);setError("");resetVerification()}} style={{padding:"15px 10px",border:`2px solid ${userType===t?C.gold:C.gray200}`,borderRadius:10,background:userType===t?C.goldBg:C.white,cursor:"pointer",fontWeight:900,color:C.dark}}>{t}</button>)}</div></div>
        {!userType ? <div style={{padding:"28px 16px",borderRadius:12,background:C.gray100,textAlign:"center",color:C.gray500,fontSize:13}}>Select <b>Banker</b>, <b>Vendor Admin</b> or <b>Vendor Employee</b> to open the registration form.</div> : <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:15}}>
          ...
          <div style={{gridColumn:"span 2"}}><label style={labelStyle}>Official Email *</label><div style={{display:"flex",gap:8}}><input style={{...inputStyle,flex:1}} type="email" value={form.email} onChange={e=>{setForm(f=>({...f,email:e.target.value}));setOtpSent(false);setVerified(false)}} placeholder={domain||"your.official@email.com"}/><button type="button" onClick={sendOtp} style={{padding:"0 16px",background:domainValid?C.gold:C.gray200,color:domainValid?C.dark:C.gray500,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>Send OTP</button></div>{domain&&<div style={{fontSize:11,color:domainValid?C.green:C.gray500,marginTop:5}}>{domainValid?"✓ Official domain verified":"Required domain: "+domain}</div>}</div>
          {otpSent&&<div style={{gridColumn:"span 2",background:C.gray100,borderRadius:9,padding:13}}><label style={labelStyle}>Enter OTP *</label><div style={{display:"flex",gap:8}}><input style={{...inputStyle,flex:1,letterSpacing:4}} inputMode="numeric" maxLength={6} value={form.otp} onChange={set("otp")} placeholder="6-digit OTP"/><button type="button" onClick={verifyOtp} style={{padding:"0 16px",background:verified?C.green:C.dark,color:C.white,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer"}}>{verified?"✓ Verified":"Verify OTP"}</button></div><div style={{fontSize:11,color:C.gray500,marginTop:7}}>Prototype: the OTP is generated in the browser console. Real email delivery will be connected later.</div></div>}
          <div><label style={labelStyle}>Set Password *</label><input style={inputStyle} type="password" value={form.password} onChange={set("password")} placeholder="Minimum 8 characters"/></div>
          <div><label style={labelStyle}>Confirm Password *</label><input style={inputStyle} type="password" value={form.confirm} onChange={set("confirm")} placeholder="Re-enter password"/></div>
          <div style={{gridColumn:"span 2"}}><button type="button" onClick={doSignup} disabled={loading} style={{width:"100%",padding:14,background:loading?C.gray300:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:900,fontSize:15,cursor:loading?"not-allowed":"pointer"}}>{loading?"Creating account…":"Create Account →"}</button></div>
        </div>}
      </div>
    </div>
  </div>;
...
function ForgotPage({ onBack }) {
  const [email,setEmail]=useState(""); const [otp,setOtp]=useState(""); const [newPassword,setNewPassword]=useState(""); const [confirm,setConfirm]=useState(""); const [sentOtp,setSentOtp]=useState(""); const [step,setStep]=useState("email"); const [error,setError]=useState("");
  const send=()=>{setError("");const e=email.trim().toLowerCase();const user=DB.get("users").find(u=>u.email?.toLowerCase()===e&&u.active);if(!user){setError("No active account found with this email.");return;}const code=makeOtp();setSentOtp(code);setStep("otp");console.log("Prototype password-reset OTP:",code)};
  const verify=()=>{setError("");if(otp!==sentOtp){setError("Invalid OTP.");return;}setStep("password")};
  const reset=async()=>{setError("");if(newPassword.length<8){setError("Password must be at least 8 characters.");return;}if(newPassword!==confirm){setError("Passwords do not match.");return;}const user=DB.get("users").find(u=>u.email?.toLowerCase()===email.trim().toLowerCase()&&u.active);if(!user){setError("Account not found.");return;}const hash=await DB.hashPassword(newPassword);DB.update("users",user.id,{passwordHash:hash});DB.audit("PASSWORD_RESET",user.id,{email:user.email});setStep("done")};
  return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20}}><div style={{background:C.white,borderRadius:16,padding:38,width:430,maxWidth:"100%",boxShadow:"0 8px 32px #0002"}}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:22}}><span onClick={onBack} style={{cursor:"pointer",color:C.gold,fontSize:22,fontWeight:700}}>←</span><h2 style={{margin:0,fontWeight:900,fontSize:22,color:C.dark}}>Forgot Password</h2></div>{error&&<div style={{background:"#fee2e2",color:C.red,padding:"10px 14px",borderRadius:8,marginBottom:14,fontSize:13}}>⚠️ {error}</div>}{step==="done"?<><div style={{background:C.greenBg,color:C.green,padding:16,borderRadius:10,marginBottom:18,fontWeight:700}}>✓ Password changed successfully.</div><button onClick={onBack} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer"}}>Back to Login</button></>:step==="email"?<><label style={labelStyle}>Official Email</label><input style={inputStyle} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="your.official@email.com"/><button onClick={send} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer",marginTop:14}}>Send OTP</button><div style={{fontSize:11,color:C.gray500,marginTop:8}}>The reset OTP will be sent to the verified email in the production setup.</div></>:step==="otp"?<><label style={labelStyle}>Enter OTP</label><input style={inputStyle} inputMode="numeric" maxLength={6} value={otp} onChange={e=>setOtp(e.target.value)} placeholder="6-digit OTP"/><button onClick={verify} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer",marginTop:14}}>Verify OTP</button><div style={{fontSize:11,color:C.gray500,marginTop:8}}>Prototype testing: OTP is available in the browser console.</div></>:<><label style={labelStyle}>New Password</label><input style={inputStyle} type="password" value={newPassword} onChange={e=>setNewPassword(e.target.value)} placeholder="Minimum 8 characters"/><label style={{...labelStyle,marginTop:12}}>Confirm Password</label><input style={inputStyle} type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} placeholder="Re-enter password"/><button onClick={reset} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer",marginTop:14}}>Set New Password</button></>}</div></div>;
}
...
// ─── AUTH PAGES ───────────────────────────────────────────────────────────────
const AUTH_ROLES = ["Admin", "Operations", "Employee", "Sales Manager", "Vendor Admin", "Vendor Employee"];
...
function LoginPage({ onLogin }) {
  ...
  return <div style={{display:"flex",minHeight:"100vh",fontFamily:"'Inter','Segoe UI',sans-serif",background:C.pageBg}}>
    <BrandPanel />
    <div style={{width:500,maxWidth:"100%",background:C.white,display:"flex",flexDirection:"column",justifyContent:"center",padding:"42px 46px",boxSizing:"border-box"}}>
      <div style={{textAlign:"center",marginBottom:24}}><div style={{fontSize:11,fontWeight:800,color:C.gold,letterSpacing:2,textTransform:"uppercase",marginBottom:8}}>SECURE ACCESS</div><h2 style={{color:C.dark,fontWeight:900,fontSize:29,margin:"0 0 6px"}}>Welcome back</h2><p style={{color:C.gray500,fontSize:13,margin:0}}>Sign in using your verified official email.</p></div>
      {error&&<div style={{background:"#fee2e2",border:"1px solid #fca5a5",borderRadius:8,padding:"10px 14px",marginBottom:16,color:C.red,fontSize:13,fontWeight:600}}>⚠️ {error}</div>}
      <div style={{marginBottom:16}}><label style={labelStyle}>Official Email</label><input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={inputStyle} placeholder="your.official@email.com" autoComplete="email" /></div>
      <div style={{marginBottom:22}}><label style={labelStyle}>Password</label><div style={{position:"relative"}}><input type={showPwd?"text":"password"} value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&doLogin()} style={{...inputStyle,paddingRight:44}} placeholder="Enter password" autoComplete="current-password"/><span onClick={()=>setShowPwd(!showPwd)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",cursor:"pointer"}}>{showPwd?"🙈":"👁️"}</span></div></div>
      <button onClick={doLogin} disabled={loading} style={{width:"100%",padding:13,background:loading?C.gray300:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:900,fontSize:15,cursor:loading?"not-allowed":"pointer"}}>{loading?"⏳ Signing in…":"Sign In →"}</button>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:16}}><span onClick={()=>{setError("");setAuthView("forgot")}} style={{color:C.gold,fontSize:13,cursor:"pointer",fontWeight:700}}>Forgot password?</span><span onClick={()=>{setError("");setAuthView("signup")}} style={{color:C.gold,fontSize:13,cursor:"pointer",fontWeight:800}}>Create New User →</span></div>
    </div>
  </div>;
}
...
function SignupPage({ onBack }) {
  ...
  const [form,setForm]=useState({name:"",bankCode:"",vertical:"",branch:"",email:"",otp:"",password:"",confirm:""});
  const [otp,setOtp]=useState(""); const [otpSent,setOtpSent]=useState(false); const [verified,setVerified]=useState(false); const [error,setError]=useState(""); const [success,setSuccess]=useState(false); const [loading,setLoading]=useState(false);
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value})); const selectedBank=getBank(form.bankCode);
  const isVendor=userType.startsWith("Vendor");
  const domain=isVendor?EMAIL_DOMAINS.VENDOR:selectedBank?.emailDomain;
  const domainValid=!!form.email.trim()&&!!domain&&emailMatchesDomain(form.email,domain);
  const sendOtp=()=>{
    setError("");
    if(!form.email.trim()){setError("Please enter your official email.");return;}
    if(!domainValid){setError(`Please use an official email ending with ${domain||"the selected bank domain"}.`);return;}
    const duplicate=DB.get("users").some(u=>u.email?.toLowerCase()===form.email.trim().toLowerCase())||DB.get("access_requests").some(r=>r.status==="Pending"&&r.email?.toLowerCase()===form.email.trim().toLowerCase());
    if(duplicate){setError("An account or pending request already exists for this email.");return;}
    const code=makeOtp(); setOtp(code); setOtpSent(true); setVerified(false); setForm(f=>({...f,otp:""}));
    console.log("Prototype email OTP:",code); // Real email sending requires a backend/email provider.
  };
  const verifyOtp=()=>{setError("");if(form.otp.trim()===otp&&otpSent){setVerified(true);setError("")}else setError("Invalid OTP. Please enter the latest 6-digit code.")};
  const doSignup=async()=>{
    setError("");
    if(!userType){setError("Please select Vendor Admin or Vendor Employee.");return;}
    if(!form.name.trim()||!form.bankCode||!form.vertical||!form.branch.trim()||!form.email.trim()){setError("Please fill all required fields.");return;}
    if(!domainValid){setError(`Official email must end with ${domain||"the selected bank domain"}.`);return;}
    if(!verified){setError("Please verify your email with OTP first.");return;}
    if(form.password.length<8){setError("Password must be at least 8 characters.");return;}
    if(form.password!==form.confirm){setError("Passwords do not match.");return;}
    const role=userType==="Vendor Admin"?"Vendor Admin":"Vendor Employee";
    const hash=await DB.hashPassword(form.password);
    const base={name:form.name.trim(),username:form.email.trim().split("@")[0],email:form.email.trim(),passwordHash:hash,role,bankCode:form.bankCode,bankName:selectedBank.name,vertical:form.vertical,branch:form.branch.trim(),emailVerified:true};
    if(role==="Vendor Admin") {
      const user=DB.insert("users",{id:`user_${Date.now()}_${Math.random().toString(36).slice(2,7)}`,...base,active:true,approved:true,approvedAt:new Date().toISOString(),createdAt:new Date().toISOString(),createdBy:"self-registration"});
  ...
  if(success) return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:24}}><div style={{background:C.white,borderRadius:16,padding:40,width:520,maxWidth:"100%",boxShadow:"0 8px 32px #0002",textAlign:"center"}}><div style={{fontSize:45}}>✅</div><h2 style={{margin:"10px 0 8px",color:C.dark}}>Request submitted</h2><p style={{color:C.gray500,fontSize:14,lineHeight:1.6}}>Your email is verified. {userType==="Vendor Employee"?"Vendor Employee access has been sent to Admin for approval.":"Your Vendor Admin account is ready. You can sign in with your verified email and password."}</p><button onClick={onBack} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:800,cursor:"pointer",marginTop:12}}>← Back to Login</button></div></div>;
  return <div style={{minHeight:"100vh",background:C.pageBg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20,boxSizing:"border-box"}}><div style={{background:C.white,borderRadius:16,padding:34,width:650,maxWidth:"100%",boxShadow:"0 8px 32px #0002"}}>
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}><span onClick={onBack} style={{cursor:"pointer",color:C.gold,fontSize:22,fontWeight:700}}>←</span><div><h2 style={{margin:0,fontWeight:900,fontSize:23,color:C.dark}}>Create New User</h2><p style={{margin:0,color:C.gray500,fontSize:12}}>Verify your official email before creating access.</p></div></div>
    {error&&<div style={{background:"#fee2e2",color:C.red,padding:"10px 14px",borderRadius:8,marginBottom:14,fontSize:13,fontWeight:600}}>⚠️ {error}</div>}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:13}}>
      <div style={{gridColumn:"span 2"}}><label style={labelStyle}>User Type *</label><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{["Vendor Admin","Vendor Employee"].map(t=><button type="button" key={t} onClick={()=>{setUserType(t);setError("");setVerified(false);setOtpSent(false)}} style={{padding:13,border:`2px solid ${userType===t?C.gold:C.gray200}`,borderRadius:9,background:userType===t?C.goldBg:C.white,cursor:"pointer",fontWeight:900,color:C.dark}}>{t}</button>)}</div></div>
      <div style={{gridColumn:"span 2"}}><label style={labelStyle}>Full Name *</label><input style={inputStyle} value={form.name} onChange={set("name")} placeholder="Full name"/></div>
      {!isVendor&&<></>}
      <div style={{gridColumn:"span 2"}}><label style={labelStyle}>Bank *</label><select style={inputStyle} value={form.bankCode} onChange={e=>{setForm(f=>({...f,bankCode:e.target.value,email:"",otp:""}));setOtpSent(false);setVerified(false);setError("")}}><option value="">Select bank</option>{BANKS.map(b=><option key={b.code} value={b.code}>{b.name}</option>)}</select></div>
      <div><label style={labelStyle}>Vertical *</label><select style={inputStyle} value={form.vertical} onChange={set("vertical")}><option value="">Select vertical</option>{verticals.map(v=><option key={v}>{v}</option>)}</select></div>
      <div><label style={labelStyle}>Branch *</label><input style={inputStyle} value={form.branch} onChange={set("branch")} placeholder="Branch / location"/></div>
      <div style={{gridColumn:"span 2"}}><label style={labelStyle}>Official Email ID *</label><div style={{display:"flex",gap:8}}><input style={{...inputStyle,flex:1}} type="email" value={form.email} onChange={e=>{setForm(f=>({...f,email:e.target.value}));setOtpSent(false);setVerified(false)}} placeholder={isVendor?"employee@arskeil.in":domain||"official@bank.com"}/><button type="button" onClick={sendOtp} style={{padding:"0 14px",background:C.gold,color:C.dark,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer",whiteSpace:"nowrap"}}>Send OTP</button></div>{domain&&<div style={{fontSize:11,color:domainValid?C.green:C.gray500,marginTop:5}}>{domainValid?"✓ Official domain verified":"Required domain: "+domain}</div>}</div>
      {otpSent&&<div style={{gridColumn:"span 2",background:C.gray100,borderRadius:9,padding:12}}><label style={labelStyle}>Enter OTP *</label><div style={{display:"flex",gap:8}}><input style={{...inputStyle,flex:1,letterSpacing:4}} inputMode="numeric" maxLength={6} value={form.otp} onChange={set("otp")} placeholder="6-digit OTP"/><button type="button" onClick={verifyOtp} style={{padding:"0 16px",background:verified?C.green:C.dark,color:C.white,border:"none",borderRadius:7,fontWeight:800,cursor:"pointer"}}>{verified?"✓ Verified":"Verify OTP"}</button></div><div style={{fontSize:11,color:C.gray500,marginTop:7}}>Prototype testing: OTP is generated in the browser console. Connect an email service/backend later for real delivery.</div></div>}
      <div><label style={labelStyle}>Set Password *</label><input style={inputStyle} type="password" value={form.password} onChange={set("password")} placeholder="Minimum 8 characters"/></div>
      <div><label style={labelStyle}>Confirm Password *</label><input style={inputStyle} type="password" value={form.confirm} onChange={set("confirm")} placeholder="Re-enter password"/></div>
      <div style={{gridColumn:"span 2"}}><button onClick={doSignup} disabled={loading} style={{width:"100%",padding:13,background:C.gold,color:C.dark,border:"none",borderRadius:8,fontWeight:900,fontSize:15,cursor:"pointer"}}>Create Account →</button></div>
    </div>
  </div></div>;
}
