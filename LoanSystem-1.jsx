...
function LoginPage({ onLogin }) {
  ...
  const resetLogin = () => {
    setError(""); setEmail(""); setPassword(""); setLoading(false);
  };
  ...
  const doLogin = async () => {
    ...
    try {
      ...
      if (profile.role === "Vendor Admin") {
        const allowedVendorAdmins = ["founder@arskeil.in", "khanbushraparvez@gmail.com"];
        if (!allowedVendorAdmins.includes(e)) {
          await supabase.auth.signOut();
          throw new Error("This email is not authorized for Vendor Admin login.");
        }
      }
  ...
  return (
    <div style={{ width: 224, minWidth: 224, background: C.sidebar, minHeight: "100vh", display: "flex", flexDirection: "column", padding: "0 0 24px 0" }}>
      ...
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 12 }}>
        <div style={{ width: 52, height: 52, borderRadius: "50%", background: C.goldBg, border: `2px solid ${C.gold}`, color: C.dark, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: 17, flexShrink: 0, overflow: "hidden" }} title="Profile picture">
          {session?.profile_pic_url || session?.avatar_url ? <img src={session.profile_pic_url || session.avatar_url} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <span>📷</span>}
        </div>
        <div style={{ minWidth: 0 }}><div style={{ color: C.white, fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{session?.name || session?.username}</div><div style={{ color: roleColor, fontSize: 10, fontWeight: 700 }}>{session?.role}</div><div style={{ color: C.gold, fontSize: 9, fontWeight: 800, marginTop: 2 }}>🏦 {session?.bankCode || "OTHERS"}</div><div style={{ color: C.gray500, fontSize: 8, marginTop: 3 }}>Profile picture</div></div>
      </div>
